...
export class AssignmentsCoreRouteComponent implements OnInit, OnDestroy {
    ...
    this._electronService.ipcRenderer.on('refactoring-exercise-response', (event, data) => {
      try {
        if (data) {
          ...
          console.error("Dati di risposta vuoti.");
        }
      } catch (err) {
        console.error("Errore durante l'elaborazione della risposta:", err);
      }
    });
  ...
  compile() {
    ...
    if(this.compileType == 1){
      ...
      this.codeEditorService.compile(this.compiledExercise, this.exerciseConfiguration).subscribe(data =>{
        this.elaborateCompilerAnswer(data);
      }, error => {
